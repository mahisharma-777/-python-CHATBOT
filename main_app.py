...
def send():
    ...
    if "play" in user_msg.lower() and "youtube" in user_msg.lower():
        ...
        try:
            search_name = user_msg.lower().replace('search', '').replace('on google', "").strip()
            if search_name!="":
                chat.insert(END, f"Bot : searching {search_name} on goole")
                speech_output.speak_text(f"Searching{search_name} on google")
                pywhatkit.search(search_name)
            else:
                chat.insert(END, f"Bot: Please tell me what you want to search on google")
                speech_output.speak_text(f"Please tell me what you want to search on google")
        except Exception as e:
            speech_output.speak_text(f"Sorry i didn't get you")
            print(e)

    elif "search" in user_msg.lower() and "google map" in user_msg.lower():
        try:
            search_name = user_msg.lower().replace('search', '').replace('on google map',"").strip()
            if search_name!="":
                chat.insert(END, f"Bot : searching {search_name} on goole map")
                speech_output.speak_text(f"Searching{search_name} on google map")
                pywhatkit.search(search_name)
            else:
                chat.insert(END, f"Bot: Please tell me what you want to search on google map")
                speech_output.speak_text(f"Please tell me what you want to search on google map")
        except Exception as e:
            speech_output.speak_text(f"Sorry i didn't get you")
            print(e)
     ...
     try:
         chat.insert(END, f"Bot: I don't know that. Searching on Google...\n")
         speech_output.speak_text("I don't know that. Searching on Google")

        # perform google search
         pywhatkit.search(user_msg)

     except Exception as e:
         chat.insert(END, "Bot: Unable to search on Google. Please teach me!\n")
         speech_output.speak_text("Unable to search on Google. Please teach me!")
         print(e)
         teach_bot(user_msg)
...
def my_app_gui():
    ...
    chat_bot_img = Image.open(r'chatbot.png')
